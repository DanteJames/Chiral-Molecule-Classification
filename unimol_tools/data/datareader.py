...
class MolDataReader(object):
    ...
    def read_data(self, data=None, is_train=True, **params):
        ...
        if isinstance(data, str):
            ...
            # load from dict
            data = pd.DataFrame({'atoms':data['atoms'], 'coordinates': data['coordinates'], 'TARGET': data['TARGET']})
            # pdb.set_trace()
        ...
        if task == 'repr':
            ...
            if is_train and task == 'multiclass':
                multiclass_cnt = data[target_cols].max().astype(int) + 1
                multiclass_cnt = multiclass_cnt['TARGET']
